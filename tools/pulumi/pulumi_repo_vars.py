#!/usr/bin/env python3
"""Export Pulumi stack outputs + optional github_vars from bmt.tfvars.json to GitHub repo variables.

Hybrid: infra-derived vars from Pulumi (pulumi stack output <name>);
manual vars (GCP_WIF_PROVIDER, BMT_DISPATCH_APP_ID) from bmt.tfvars.json "github_vars" if present,
else not set (user sets them in GitHub). Run from repo root.
"""
...
from tools.repo.vars_contract import INFRA_OUTPUT_TO_VAR, REPO_VARS_CONTRACT
...
def get_expected_repo_vars_from_pulumi() -> dict[str, str]:
    """Return GitHub var name -> value: Pulumi for infra vars, github_vars for manual vars when present, else contract defaults."""
    ...
    for name in REPO_VARS_CONTRACT.all_var_names():
        if name in var_to_infra:
            infra_name = var_to_infra[name]
            result[name] = _pulumi_output_raw(infra_name)
        else:
...
# Keep backward compat alias for gh_repo_vars.py import
get_expected_repo_vars_from_terraform = get_expected_repo_vars_from_pulumi
