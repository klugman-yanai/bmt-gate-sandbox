...
class BmtRunLocal:
    """Local SK batch runner (config-first). No CLI; configure via env or pass to run()."""
    ...
    def run(
        ...
        *,
        ...
        opts = RunOptions(
            ...
            set_ref_to_mics=set_ref_to_mics,
            num_source_test=num_source_test,
        )
        return main(**opts.__dict__)
...
if __name__ == "__main__":
    ...
    raise SystemExit(
        BmtRunLocal().run(
            ...
            runner=s("BMT_RUNNER"),
            template=s("BMT_TEMPLATE"),
            dataset_root=s("BMT_DATASET_ROOT"),
            output_root=s("BMT_OUTPUT_ROOT"),
            log_root=s("BMT_LOG_ROOT"),
            comparison=s("BMT_COMPARISON"),
            run_root=s("BMT_RUN_ROOT", "local_batch"),
            limit=_int_env("BMT_LIMIT"),
            workers=max(1, min(_int_env("BMT_WORKERS", 1), 256)),
            timeout_sec=_int_env("BMT_TIMEOUT_SEC"),
            summary_out=s("BMT_SUMMARY_OUT", "summary.json"),
            set_ref_to_mics=truthy(e("BMT_SET_REF_TO_MICS")),
            num_source_test=int(num_src) if num_src and num_src.strip().isdigit() else None,
        )
    )
