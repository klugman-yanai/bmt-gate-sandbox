...
class WorkspaceLayout:
    ...
    Attributes:
        ...
        data_root:   data/      — local WAV corpora for bmt_run_local (legacy).

    Note: ``bmt_workspace`` (VM-side ~/bmt_workspace/) is NOT part of WorkspaceLayout.
    It is used by vm_watcher.py and bmt_manager_base.py on the running VM and has no
    presence in dev tooling. The local dev equivalent (local_batch/) is configured
    separately via BMT_LOCAL_BATCH_DIR or the --workspace flag.
    """
