...
`deploy/code` manual sync should populate `<code-root>` before VM runs.
...
3. Roll back bootstrap mode if needed:
   - `./deploy/code/bootstrap/rollback_vm_startup_to_inline.sh`

## Auth model

Watcher token resolution is repository-aware via `deploy/code/lib/github_auth.py` + `deploy/code/config/github_repos.json`.
