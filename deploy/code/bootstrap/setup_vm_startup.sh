...
if [[ -z "$GCP_PROJECT" || -z "$GCP_ZONE" || -z "$BMT_VM_NAME" || -z "$GCS_BUCKET" ]]; then
  ...
  echo "Optional: BMT_REPO_ROOT." >&2
  echo "Example: GCP_PROJECT=p GCP_ZONE=z BMT_VM_NAME=v GCS_BUCKET=b $0" >&2
  exit 1
fi

CODE_WRAPPER_URL="gs://${GCS_BUCKET}/code/bootstrap/startup_wrapper.sh"
...
echo "Setting VM metadata and startup-script-url for $BMT_VM_NAME (bucket=$GCS_BUCKET)..."
