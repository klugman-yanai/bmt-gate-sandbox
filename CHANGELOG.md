...
- `plugin_config.forced_wav_path_keys_exclude` (`list[str]`): new per-leg opt-out for the per-case WAV-path rewrite. Keys listed here are skipped in both the `_FORCED_WAV_PATH_KEYS` loop *and* the placeholder-based walk in `runtime.kardome_runparams._rewrite_json_paths_for_wav`, so they keep whatever value the template shipped. Both SK manifests now set `"forced_wav_path_keys_exclude": ["REF_PATH"]`. The SK template's `REF_PATH` stays at `/tmp/dummy/ref.wav`, which makes `tinywav_open_read` fail on the C side (`is_ref == -1`) and lets the refs channel guard added in `sanity_tests.c` short-circuit instead of refusing to run every 8-channel mics WAV against a `num_of_refs * 3 == 6` refs buffer. Other projects are unaffected (default is the empty set — historical forced-rewrite behavior).
