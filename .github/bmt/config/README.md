...
CI does **not** read any `.env` file. Variables are supplied by the workflow’s `env:` block from GitHub repo variables/secrets. Defaults for optional vars (e.g. `BMT_PROJECTS`, `BMT_HANDSHAKE_TIMEOUT_SEC`) live in the CLI `shared/config.py` code.
