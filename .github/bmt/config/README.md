...
CI does **not** read any `.env` file. Variables are supplied by the workflow’s `env:` block from GitHub repo variables/secrets. Required vars such as `BMT_STATUS_CONTEXT`, `BMT_HANDSHAKE_TIMEOUT_SEC`, `BMT_PROJECTS` are set from Terraform via `just terraform-export-vars-apply` in bmt-gcloud; the CLI `shared/config.py` still provides fallback defaults when env is missing (e.g. local runs).
...
Required variables include: `GCS_BUCKET`, `GCP_WIF_PROVIDER`, `GCP_SA_EMAIL`, `GCP_PROJECT`, `GCP_ZONE`, `BMT_VM_NAME`, and Terraform-exported vars (`BMT_STATUS_CONTEXT`, `BMT_HANDSHAKE_TIMEOUT_SEC`, `BMT_PROJECTS`). Optional and secrets are documented in bmt-gcloud.
