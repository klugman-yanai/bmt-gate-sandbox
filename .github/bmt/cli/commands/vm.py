...
def run_select_available_vm() -> None:
    ...
    if not pool:
        if not (cfg.bmt_vm_name and cfg.bmt_vm_name.strip()):
            gh_error("BMT VM pool is empty and BMT_VM_NAME is not set. Set BMT_VM_POOL (comma-separated) or BMT_VM_POOL_LABEL to list by label, or BMT_VM_NAME.")
            raise RuntimeError("BMT VM pool must not be empty; set BMT_VM_POOL, BMT_VM_POOL_LABEL, or BMT_VM_NAME.")
    ...
    if not pool:
        gh_error("BMT VM pool is empty. Set BMT_VM_POOL (comma-separated names) or BMT_VM_POOL_LABEL (e.g. bmt-gate:true) to discover VMs via Compute API.")
