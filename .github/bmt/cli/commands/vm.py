...
def run_select_available_vm() -> None:
    """Select a VM from workflow-defined pool (or BMT_VM_NAME as single-VM fallback).
    ...
    if pool_raw:
        ...
        if len(pool) < 2:
            gh_warning(
                "Workflow VM pool contains fewer than 2 entries; concurrency is limited. "
                f"Configured VM count={len(pool)}."
            )
    else:
        pool = [cfg.bmt_vm_name]
        print("Workflow VM pool is not configured; using single VM from BMT_VM_NAME.")
