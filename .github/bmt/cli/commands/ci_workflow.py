"""CI workflow commands (replace ci_workflow.sh). Used by build-and-test.yml."""
...
def _binary_dir_for_configure(presets: list[dict[str, object]], configure: str) -> str:
    for p in presets:
        if str(p.get("name", "")) == configure:
            raw = str(p.get("binaryDir", ""))
            return raw.replace("${sourceDir}", ".").strip()
    raise RuntimeError(f"Configure preset not found: {configure}")


def run_stage_release_runner() -> None:
    """Stage runner payload for BMT: create Runners/Kardome dirs and copy or placeholder kardome_runner + lib."""
    cfg = _get_configure()
...
def run_compute_preset_info() -> None:
    """Write preset, project, runners_dir, lib_dir to GITHUB_OUTPUT from MATRIX_CONFIGURE."""
    cfg = _get_configure()
    ...
    with Path(out).open("a", encoding="utf-8") as f:
        ...
        f.write(f"runners_dir={runners_dir}\n")
        f.write(f"lib_dir={lib_dir}\n")
