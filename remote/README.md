...
Any extra top-level directory (for example `remote/vmfs`, `remote/bucket`, `remote/bootstrap`, `remote/sk`) is policy drift.
...
- Put runner binaries and optional seed datasets in `remote/runtime`.
...
- Pre-commit advisory hook (`scripts/pre-commit-sync-remote.sh`) warns when `remote/` and bucket manifests diverge.
