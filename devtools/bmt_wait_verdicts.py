...
def _snapshot_verdict_uri(bucket_root: str, results_prefix: str, run_id: str) -> str:
    cleaned = results_prefix.rstrip("/")
    safe = _sanitize_run_id(run_id)
    return f"{bucket_root}/{cleaned}/snapshots/{safe}/ci_verdict.json"


def _current_pointer_uri(bucket_root: str, results_prefix: str) -> str:
...
def _parse_manifest(manifest_json: str, config_root: Path, bucket_root: str) -> list[TriggerLeg]:
    ...
    for item in data.get("legs", []):
        ...
        if item.get("results_prefix") and item.get("verdict_uri"):
            ...
            verdict_uri = _snapshot_verdict_uri(bucket_root, results_prefix, run_id)
        legs.append(
            TriggerLeg(
                project=project,
                bmt_id=bmt_id,
...
def _collect_verdict(leg: TriggerLeg, bucket: str) -> LegOutcome:
    verdict_payload, verdict_error = gcloud.download_json(leg.verdict_uri)
...
def _current_pointer_latest(bucket_root: str, results_prefix: str) -> str | None:
    uri = _current_pointer_uri(bucket_root, results_prefix)
    payload, _ = gcloud.download_json(uri)
...
def run(args: argparse.Namespace) -> None:
    ...
    write_github_output(github_output, "warning_count", str(counts[STATUS_WARNING]))
    write_github_output(github_output, "fail_count", str(counts[STATUS_FAIL]))
    write_github_output(github_output, "timeout_count", str(counts[STATUS_TIMEOUT]))
