#!/usr/bin/env python3
"""Validate canonical remote/ layout as a 1:1 bucket mirror."""
...
def main() -> int:
    ...
    if not root.is_dir():
        click.echo("::error::Missing remote/ directory", err=True)
        return 1
    ...
    if forbidden_hits:
        click.echo("::error::Forbidden runtime/generated paths found in remote/code:", err=True)
    ...
    if forbidden_runtime_hits:
        click.echo("::error::Forbidden generated/runtime paths found in remote/runtime:", err=True)
    ...
    if unexpected_top_level:
        ...
        for name in unexpected_top_level:
            click.echo(f"  - unexpected: remote/{name}", err=True)
        missing = True

    if missing:
        return 1

    click.echo("Remote layout policy check passed")
