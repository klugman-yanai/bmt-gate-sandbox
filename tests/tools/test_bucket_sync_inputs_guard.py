"""Tests for local_digest() inputs guard and resolve_local_path() routing.
...
- 0.2b: resolve_local_path() routes projects/shared/ to code_root
- 0.2b: resolve_local_path() routes projects/<other>/ to runtime_root
"""
...
resolve_local_path = batch.resolve_local_path
...
def test_local_digest_excludes_wav_under_inputs(staging_tree: Path) -> None:
    ...
    digest, count = local_digest(staging_tree, False, FORBIDDEN_RUNTIME_SEED)
...
def test_local_digest_includes_keep_under_inputs(staging_tree: Path) -> None:
    ...
    digest_with_keep, count_with_keep = local_digest(staging_tree, False, FORBIDDEN_RUNTIME_SEED)
    # Remove .keep and recompute
    (staging_tree / "projects" / "sk" / "inputs" / "false_rejects" / ".keep").unlink()
    digest_without_keep, count_without_keep = local_digest(staging_tree, False, FORBIDDEN_RUNTIME_SEED)
...
def test_local_digest_includes_manifest_under_inputs(staging_tree: Path) -> None:
    ...
    digest_with, count_with = local_digest(staging_tree, False, FORBIDDEN_RUNTIME_SEED)
    (staging_tree / "projects" / "sk" / "inputs" / "false_rejects" / "dataset_manifest.json").unlink()
    digest_without, count_without = local_digest(staging_tree, False, FORBIDDEN_RUNTIME_SEED)
...
def test_local_digest_wav_does_not_affect_digest(staging_tree: Path) -> None:
    ...
    digest_before, count_before = local_digest(staging_tree, False, FORBIDDEN_RUNTIME_SEED)
    extra_wav = staging_tree / "projects" / "sk" / "inputs" / "false_rejects" / "extra.wav"
    extra_wav.write_bytes(b"\x52\x49\x46\x46" + b"\x00" * 40)
    digest_after, count_after = local_digest(staging_tree, False, FORBIDDEN_RUNTIME_SEED)
...
# ── resolve_local_path() routing ──────────────────────────────────────────────


def test_resolve_projects_shared_routes_to_code_root(tmp_path: Path) -> None:
    """projects/shared/ paths route to code_root (gcp/image)."""
    code_root = tmp_path / "gcp" / "image"
    runtime_root = tmp_path / "gcp" / "stage"
    result = resolve_local_path("projects/shared/input_template.json", code_root, runtime_root)
    assert result == (code_root / "projects" / "shared" / "input_template.json").resolve()


def test_resolve_projects_sk_routes_to_runtime_root(tmp_path: Path) -> None:
    """projects/<project>/ paths (non-shared) route to runtime_root (gcp/stage)."""
    code_root = tmp_path / "gcp" / "image"
    runtime_root = tmp_path / "gcp" / "stage"
    result = resolve_local_path("projects/sk/kardome_runner", code_root, runtime_root)
    assert result == (runtime_root / "projects" / "sk" / "kardome_runner").resolve()


def test_resolve_projects_inputs_routes_to_runtime_root(tmp_path: Path) -> None:
    """projects/sk/inputs/... routes to runtime_root."""
    code_root = tmp_path / "gcp" / "image"
    runtime_root = tmp_path / "gcp" / "stage"
    result = resolve_local_path("projects/sk/inputs/false_rejects", code_root, runtime_root)
    assert result == (runtime_root / "projects" / "sk" / "inputs" / "false_rejects").resolve()


def test_resolve_sk_config_routes_to_code_root(tmp_path: Path) -> None:
    """Legacy sk/config/ paths route to code_root."""
    code_root = tmp_path / "gcp" / "image"
    runtime_root = tmp_path / "gcp" / "stage"
    result = resolve_local_path("sk/config/something.json", code_root, runtime_root)
    assert result == (code_root / "sk" / "config" / "something.json").resolve()


def test_resolve_absolute_path_returned_as_is(tmp_path: Path) -> None:
    code_root = tmp_path / "gcp" / "image"
    runtime_root = tmp_path / "gcp" / "stage"
    abs_path = str(tmp_path / "some" / "binary")
    result = resolve_local_path(abs_path, code_root, runtime_root)
    assert result == Path(abs_path).resolve()


def test_resolve_gs_uri_raises(tmp_path: Path) -> None:
    code_root = tmp_path / "gcp" / "image"
    runtime_root = tmp_path / "gcp" / "stage"
    with pytest.raises(ValueError, match="gs://"):
        resolve_local_path("gs://my-bucket/projects/sk/kardome_runner", code_root, runtime_root)
