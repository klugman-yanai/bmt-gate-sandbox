...
def test_resolve_uploaded_projects_uses_uploaded_markers(
    ...
    monkeypatch.setattr(
        ...
        "list_prefix",
        lambda prefix: [
            "gs://bucket-a/runtime/_workflow/uploaded/12345/sk.json",
            "gs://bucket-a/runtime/_workflow/uploaded/12345/qa.json",
        ],
    )
    monkeypatch.setattr(workflow.gcs, "download_json", lambda uri: (None, "not found"))
...
def test_resolve_uploaded_projects_accepts_preseeded_bucket_runner(
    ...
    monkeypatch.setattr(workflow.gcs, "list_prefix", lambda prefix: [])
    ...
    monkeypatch.setattr(workflow.gcs, "download_json", fake_download_json)
