...
def test_build_plan_raises_on_duplicate_results_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_build_plan_accepts_unique_results_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    stage_root = tmp_path / "stage"
    _write_project_manifest(stage_root / "projects" / "acme" / "project.json", "acme")
    _write_manifest(
        stage_root / "projects" / "acme" / "bmts" / "bmt_a" / "bmt.json",
