...
`gcp/image` sync is used for image baking inputs and tooling. Runtime VM boot does not sync code from GCS.

## Scripts

Scripts under `scripts/` expect the **root project to be installed** so `gcp.image` is importable (e.g. run from repo root with `uv run python gcp/image/scripts/<script>.py` or `uv run python -m gcp.image.scripts.<module>` where the module is a package). On the VM, the same holds: install with `pip install -e .` or `uv sync` from the image code root so imports work without path hacks.
...
| `install_deps.py` | Install the gcp/image project in editable mode with `[vm]` extra; import check verifies `config.bmt_config` and runtime deps. |
...
Path constants (on-image and bucket) live in `gcp/image/path_utils.py`.
...
3. Roll back startup script if needed:
   - `uv run python -m gcp.image.scripts.rollback_startup_to_inline`

## Auth model

Watcher token resolution is repository-aware via `gcp/image/github/github_auth.py` + `gcp/image/config/github_repos.json`.
