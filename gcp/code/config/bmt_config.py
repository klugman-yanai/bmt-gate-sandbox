...
Supports optional context file (.bmt/context.json) for config + workflow step outputs to avoid env vars in CLI."""
...
# ---------------------------------------------------------------------------
DEFAULT_REPO_ROOT: Final[str] = "/opt/bmt"
...
class BmtConfig(BaseModel):
    ...
    bmt_pubsub_topic: str = Field(default="", description="Pub/Sub topic for trigger notifications")
    bmt_pubsub_subscription: str = Field(default="", description="Pub/Sub subscription name")
    # Defaults (code only; no JSON, no env overlay)
    bmt_status_context: str = Field(default="BMT Gate", description="GitHub status check context name")
