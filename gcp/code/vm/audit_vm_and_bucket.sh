...
_log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [audit] $*"; }
...
if [[ -z "$GCP_PROJECT" || -z "$GCP_ZONE" || -z "$BMT_VM_NAME" || -z "$GCS_BUCKET" ]]; then
  echo "::error::Set GCP_PROJECT, GCP_ZONE, BMT_VM_NAME, and GCS_BUCKET." >&2
  exit 1
fi
...
echo ""
_log "=== Bucket layout (expected: code/ + runtime/) ==="
...
echo ""
_log "=== Bloat: consider removing old run triggers (keeps bucket small) ==="
