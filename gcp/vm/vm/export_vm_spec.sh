...
# Optional:
#   BMT_EXPORT_DIR (default: ./gcp/vm/vm/out)
#
# Example:
#   export GCP_PROJECT=... GCP_ZONE=europe-west4-a BMT_VM_NAME=bmt-performance-gate
#   ./gcp/vm/vm/export_vm_spec.sh

set -euo pipefail

_log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [export_vm_spec] $*"; }
_log_err() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [export_vm_spec] $*" >&2; }

GCP_PROJECT="${GCP_PROJECT:-}"
GCP_ZONE="${GCP_ZONE:-}"
BMT_VM_NAME="${BMT_VM_NAME:-}"
BMT_EXPORT_DIR="${BMT_EXPORT_DIR:-./gcp/vm/vm/out}"
...
if ! command -v jq >/dev/null 2>&1; then
	_log_err "::error::jq is required for export_vm_spec.sh."
	exit 1
fi
...
_log "Exporting VM spec for ${BMT_VM_NAME} (${GCP_PROJECT}/${GCP_ZONE})..."
...
_log "Wrote VM spec JSON: ${json_path}"
_log "Wrote VM summary : ${summary_path}"
