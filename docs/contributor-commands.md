...
- create **`gcp/stage/projects/<project>/`** (if it does not exist yet), and optionally
...
Pushing the **entire** `gcp/` mirror to the bucket is **`just sync-to-bucket`**, not `stage publish` (that syncs **one project** subtree during publish).
...
| Argument | Meaning |
| -------- | ------- |
| **`project`** | The **project name** — same segment as in `gcp/stage/projects/<project>/` (e.g. `myproject`). Data lands under `projects/<project>/inputs/<dataset>/` in GCS. |
...
- **`--local`** — Also mirror into `gcp/stage/` (default is **GCS only**; datasets can be huge).
...
| ---------- | -------------------------- |
| **`deploy`** | Upload local **`gcp/`** to your **GCS bucket** and verify — **run after you edit config under `gcp/`** and need CI to see it. Shorthand: **`just sync-to-bucket`**. |
| **`pulumi`** | Pulumi apply + push GitHub repo vars (infra). |
| **`validate`** | Compare GitHub repo variables to what Pulumi expects. |
| **`preflight`** | Bucket vs `gcp/image` checks (and optional image build); heavy sanity check before ship. |
| **`e2e`** | Local E2E readiness / preflight toward a real handoff (see help). |

**Most contributors upload to the bucket often** once a project exists: edit under `gcp/stage/…`, then **`just sync-to-bucket`** (same as **`just workspace deploy`**) so the cloud copy matches your repo.
