...
| **Commit statuses: Read and write** | Repository | Used by: (1) **bmt.yml** failure paths; (2) **dummy-build-and-test.yml** trigger failure path; (3) **VM (vm_watcher.py)** for runtime progress context (default `BMT Runtime`) and terminal gate context (default `BMT Gate`) via `POST /repos/{owner}/{repo}/statuses/{sha}`. Branch protection should require only the gate context. |
...
| **Statuses (R/W)**| CI, BMT workflow, VM         | VM posts runtime progress + final gate statuses; workflows post failure statuses on handoff errors. |
