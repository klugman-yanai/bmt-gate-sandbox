...
| **Commit statuses** | Workflows/apps can post status via API; branch protection can require a status. | BMT keeps merge gating in `BMT_STATUS_CONTEXT` (default `BMT Gate`) and uses `BMT_RUNTIME_CONTEXT` (default `BMT Runtime`) for runtime progress. |
...
- `BMT_RUNTIME_CONTEXT` is used for pending/progress runtime updates.
