...
This document describes how the system runs today, with the current split storage contract and manual code sync model.
...
1. `run_trigger.py` writes trigger payload with explicit `bucket_prefix_parent`, `code_prefix`, `runtime_prefix`.
