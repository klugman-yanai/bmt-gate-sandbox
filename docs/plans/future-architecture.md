...
| VM shared libs | `remote/lib/`: `github_auth.py`, `github_checks.py`, `status_file.py` | Add `remote/lib/bmt_lib/` (gcs, cache, gate, models) and `remote/lib/github_api.py` (PyGithub, tabulate) |
...
Config and runtime data would move from `dict[str, Any]` to Pydantic models in `remote/lib/bmt_lib/models.py` (and CI could use matching models):
...
- **Check run:** Already implemented via `remote/lib/github_checks.py` and watcher. Planned: optional migration to `github_api.py` + PyGithub.
...
- **Repo ↔ VM path:** Add mapping for `remote/lib/bmt_lib/` → `/opt/bmt/lib/bmt_lib/` and `remote/lib/github_api.py` → `/opt/bmt/lib/github_api.py`.
- **Sync script:** e.g. `devtools/sync_to_vm.sh` using `gcloud compute scp --recurse` and remote script to arrange files under `/opt/bmt/` (bin, lib, managers, config, templates, data, cache, runtime).
