...
| `BMT_PREEMPT_ON_PR_STALE_QUEUE` | `"1"` | For PR runs, allow preflight cleanup/restart only for stale triggers from the **same PR**. Triggers from different PRs are preserved for isolation. |
