...
| **`runtime/kardome_runparams.py`** (image) | Invokes runner runparams CLI (`--input-wav` / `--user-output` / toggles) and reads sidecar `.bmt.json`; metric field name and JSON key aliases are plugin-configurable (`metric_name`, `metric_json_keys`). |
| **`runtime/kardome_case_metrics.py`** (image) | Sidecar filename/key extraction helpers. Default aliases match SK (`namuh_*`), but any project can provide its own metric keys via manifest `plugin_config.metric_json_keys`. |
