...
from bmt_gate import config, core
from bmt_gate.actions import gh_error, gh_notice, gh_warning, write_github_output
...
class VmManager:
    ...
    def sync_metadata(self) -> None:
        """Sync VM metadata and inline startup script from bmt_gate.resources."""
        self._cfg.require_gcp()
        project = self._cfg.gcp_project
        zone = self._cfg.gcp_zone
        instance_name = self._cfg.bmt_vm_name
        ...
        try:
            ...
            raise RuntimeError(
                "Missing packaged startup entrypoint: bmt_gate.resources/startup_entrypoint.sh"
            ) from exc
