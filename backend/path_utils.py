"""On-image and bucket path constants for backend scripts and VM runtime.

Single source of truth for paths used by backend code and scripts. tools/repo/paths.py
remains the source for repo layout (DEFAULT_CONFIG_ROOT = backend); this module
describes what lives inside that mirror and on the VM. Repo root default comes from
backend.config.bmt_config (single source of truth for BMT defaults).
"""
...
# ---------------------------------------------------------------------------
# Code is not in GCS; it is baked into the image via Packer (from backend/).
