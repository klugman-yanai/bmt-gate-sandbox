...
def _leg_identity(leg: PlanLeg) -> dict[str, str]:
    return {
        "project": leg.project,
        "bmt_slug": leg.bmt_slug,
        "bmt_id": leg.bmt_id,
...
def _load_coordinator_summaries(
    *,
    ...
    missing_legs = [
        leg for leg in plan.legs if summary_path(workflow_run_id, leg.project, leg.bmt_slug) not in present_summary_paths
    ]
    extra_summary_paths = sorted(path for path in present_summary_paths if path not in expected_summary_paths)
    if log_completeness_warning and (missing_legs or extra_summary_paths):
        logger.warning(
            "coordinator completeness mismatch workflow_run_id=%s missing_legs=%s extra_summary_paths=%s",
            workflow_run_id,
            [_leg_identity(leg) for leg in missing_legs],
            extra_summary_paths,
        )
    return [
        load_summary_or_failure(
...
def run_coordinator_mode(*, workflow_run_id: str, stage_root: Path | None = None) -> int:
    ...
    try:
        plan = load_plan(stage_root=runtime.stage_root, workflow_run_id=workflow_run_id)
        summaries = _load_coordinator_summaries(
            runtime=runtime,
            plan=plan,
            workflow_run_id=workflow_run_id,
            ...
            if not publish_done:
                try:
                    summaries = _load_coordinator_summaries(
                        ...
                        plan=plan,
                        workflow_run_id=workflow_run_id,
                        log_completeness_warning=False,
                    )
                    publish_final_results(plan=plan, summaries=summaries, runtime=runtime)
