...
from backend.config.constants import ENV_BMT_KARDOME_CASE_TIMEOUT_SEC
...
from backend.runtime.stdout_counter_parse import counter_pattern_from_parsing_dict, read_counter_from_log

logger = logging.getLogger(__name__)

# Placeholder prefix used in runner_input.template.json; paths starting with this are rewritten per-WAV.
        ...
        try:
            ...
            try:
                ...
                return CaseResult(
                    ...
                    runner_case_diagnostic=f"log_open_failed:{type(exc).__name__}:{exc}",
                )
            try:
                try:
                    proc = subprocess.run(
                    ...
                    return CaseResult(
                        ...
                        runner_case_diagnostic=f"runner_os_error:{type(exc).__name__}:{exc}",
                    )
                ...
                return CaseResult(
                    ...
                    status=CaseStatus.OK if ok else CaseStatus.FAILED,
                    metrics=CaseMetrics(root={"namuh_count": float(counter if counter is not None else 0)}),
                    artifacts=CaseArtifacts(
                        root={
                            "log_path": str(log_path),
                            "output_path": str(output_path),
            ...
            return ExecutionResult(
                ...
                case_results=[
                    CaseResult(
                        ...
                        runner_case_diagnostic="dataset_root_missing_or_not_a_directory",
                    )
                ],
            )
            ...
            return ExecutionResult(
                ...
                case_results=[
                    CaseResult(
                        ...
                        metrics=CaseMetrics(root={}),
                        artifacts=CaseArtifacts(root={}),
                        runner_case_diagnostic=f"template_load_failed:{type(exc).__name__}:{exc}",
                    )
                ],
            )
