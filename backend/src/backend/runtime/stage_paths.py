...
def resolve_plugin_workspace_dir(stage_root: Path, project: str, plugin_name: str) -> Path:
    """Editable plugin tree.

    1. ``plugins/<name>/workspace/`` (v2)
    2. ``plugin_workspaces/<name>/`` (v1)
    3. **Flat project-root:** ``projects/<project>/plugin.json`` (one plugin per project;
       ``plugin_name`` is ignored; use ``workspace:main`` in manifests).
    4. Fallback: v1 path (may not exist).
    """
