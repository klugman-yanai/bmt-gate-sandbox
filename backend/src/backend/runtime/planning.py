...
from backend.runtime.plugin_loader import _resolve_plugin_root
from backend.runtime.plugin_publisher import plugin_digest
from backend.runtime.stage_paths import iter_bmt_manifest_paths
...
def _ensure_unique_results_paths(legs: list[PlanLeg]) -> None:
    by_results_path: dict[str, list[PlanLeg]] = {}
    for leg in legs:
        by_results_path.setdefault(str(leg.results_path), []).append(leg)
    duplicates = {path: path_legs for path, path_legs in by_results_path.items() if len(path_legs) > 1}
    if not duplicates:
        return

    duplicate_details = []
    for results_path, path_legs in sorted(duplicates.items()):
        leg_details = ", ".join(
            f"{leg.project}/{leg.bmt_slug} ({leg.bmt_id})"
            for leg in sorted(path_legs, key=lambda item: (item.project, item.bmt_slug, item.bmt_id))
        )
        duplicate_details.append(f"{results_path}: {leg_details}")
    detail = "; ".join(duplicate_details)
    raise ValueError(f"Duplicate results_path values in execution plan: {detail}")
...
def build_plan(*, runtime: StageRuntimePaths, options: PlanOptions) -> ExecutionPlan:
    ...
    _ensure_unique_results_paths(legs)
