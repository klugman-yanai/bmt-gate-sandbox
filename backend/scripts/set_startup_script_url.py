...
from backend.path_utils import DEFAULT_BMT_REPO_ROOT, SCRIPTS_STARTUP_ENTRYPOINT
...
def main() -> int:
    ...
    if not all([project, vm_name, bucket]):
        ...
        print("Optional: BMT_REPO_ROOT. Example: GCP_PROJECT=p BMT_LIVE_VM=v GCS_BUCKET=b", file=sys.stderr)
        return 1
    ...
    print("Rollback: python -m backend.scripts.rollback_startup_to_inline")
    return 0
