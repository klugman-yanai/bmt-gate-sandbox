#!/usr/bin/env bash
# Pre-commit hook: if deploy/ changed, require that deploy/ is in sync with GCS
...
if [[ -z "${GCS_BUCKET:-}" ]]; then
  echo "deploy/ changed but GCS_BUCKET is not set; sync cannot be verified."
  echo "Set GCS_BUCKET and run: just sync-remote && just verify-sync"
...
if [[ "$code_ok" -ne 1 || "$runtime_ok" -ne 1 ]]; then
  echo "deploy/ is out of sync with bucket; BMT workflow may fail (VM will accept zero legs)."
  echo "Run: just sync-remote && just verify-sync"
