...
2. **Pulumi exports repo variables** as part of `just workspace pulumi`. Re-run it whenever outputs change:

   ```bash
   just workspace pulumi
   ```

3. **Set secrets manually** (repo variables, including `GCP_WIF_PROVIDER`, are synced by `just workspace pulumi` from `bmt.config.json`):
...
GitHub reporting selects the credential profile from the repository (`owner/repo`):
...
- **`bootstrap_gh_vars.sh`** — Applies GitHub repo variables and secrets from an env file. Run after `just workspace pulumi` to set secrets, or to apply all vars from a single env file.
