...
class PresetManager:
    ...
    def stage_release_runner(self) -> None:
        ...
        if runner_bin.is_file():
            ...
            return
        sk_runner = Path("benchmarks/sk/runners/sk_gcc_release/kardome_runner")
        sk_lib = Path("benchmarks/sk/runners/lib/libKardome.so")
        if sk_runner.is_file() and project == "sk":
            ...
            print("Using real runner from benchmarks/sk/runners/ (production-like artifact)")
            return
        runner_bin.touch()
